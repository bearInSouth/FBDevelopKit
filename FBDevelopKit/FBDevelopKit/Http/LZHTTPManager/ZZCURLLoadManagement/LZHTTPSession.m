...
- (instancetype)init{
    if ([super init]) {
        ...
        _allBaseStringArr = baseStrArr ? [baseStrArr mutableCopy] : [@[@"https://newapi.ifreegroup.net/index.php/"] mutableCopy];
...
- (void)request:(LZHTTPSessionSignal *)signal refresh:(BOOL)isRefresh{
    ...
    if (self.HookParamsEncryptBlock) {
        self.HookParamsEncryptBlock([mutParas copy], ^(NSDictionary<NSString *,NSString *> * _Nonnull headerParams, NSDictionary<NSString *,NSString *> * _Nonnull bodyParams) {
            [headerParams enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSString * _Nonnull obj, BOOL * _Nonnull stop) {
                [self->_sessionManager.requestSerializer setValue:obj forHTTPHeaderField:key];
            }];
            
            [mutParas addEntriesFromDictionary:bodyParams];
        });
    }
...
+ (LZHTTPSessionSignal *)signalWithUrlId:(NSString *)url_id maker:(void (^)(LZHTTPRequestMaker * _Nonnull))makeBlock{
    LZHTTPSessionSignal *signal = [self getSignalWithUrlId:url_id];
    
    if (!signal) {
        signal = [self creatSignalWithUrlId:url_id maker:makeBlock];
    }else{
        [signal updateWithMaker:makeBlock];
    }
...
+ (LZHTTPSessionSignal *)creatSignalWithUrl:(NSString *)url maker:(void (^)(LZHTTPRequestMaker * _Nonnull))makeBlock{
    ...
    NSString *url_id = [self MD5Encode:url];
...
+ (LZHTTPSessionSignal *)getSignalWithUrl:(NSString *)url{
    NSString *url_id = [self MD5Encode:url];
...
+ (LZHTTPSessionSignal *)signalWithUrl:(NSString *)url maker:(void (^)(LZHTTPRequestMaker * _Nonnull))makeBlock{
    
    LZHTTPSessionSignal *signal = [self getSignalWithUrl:url];
    
    if (!signal) {
        signal = [self creatSignalWithUrl:url maker:makeBlock];
    }else{
        [signal updateWithMaker:makeBlock];
    }
